### Fix Record - 2025-11-30 (续2)

-   **Problem Description:** Linux.do 登录的用户信息保存位置或获取有问题，导致登录后页面刷新无法识别已登录状态
-   **Root Cause:** 
    1. 当用户通过 Linux.do OAuth 登录时，如果用户没有 Supabase 账户，会创建一个独立的 Linux.do profile（`is_linuxdo_only = true`）
    2. `App.tsx` 中的 `currentUser` 状态只检查 Supabase 的 auth 用户（通过 `authService.getCurrentUser()`）
    3. 对于独立的 Linux.do profile，没有对应的 Supabase auth 用户，所以 `currentUser` 仍然是 `null`
    4. 页面刷新后，虽然 profile 已经创建并保存到数据库，但是应用无法识别用户已登录，因为 `getCurrentUser()` 只返回 Supabase auth 用户
    5. 回调成功后虽然保存了数据到 localStorage，但页面加载时没有检查并恢复 Linux.do 登录状态
-   **Solution:** 
    1. **添加 Linux.do 登录状态检查方法**：
       - 在 `authService.ts` 中添加 `getLinuxDoLoginStatus()` 方法
       - 该方法从 localStorage 中读取保存的 Linux.do 数据，并根据 `user_id` 查找对应的 profile
    2. **修改 App.tsx 用户状态管理**：
       - 添加 `linuxDoProfile` 状态来存储 Linux.do 用户的 profile
       - 在初始化 auth 时，如果没有 Supabase 用户，检查是否有 Linux.do 登录状态
       - 如果检测到 Linux.do profile，设置 `linuxDoProfile` 状态
    3. **确保回调成功后保存登录状态**：
       - 修改 `linuxDoCallbackService.ts` 中的 `findOrCreateLinuxDoProfile` 函数
       - 在成功创建/更新 profile 后，确保将登录信息保存到 localStorage（用于页面刷新后恢复状态）
       - 修改 `App.tsx` 中的回调处理逻辑，在登录成功后保存 profile 到状态和 localStorage
-   **Impact Scope:**
    -   `services/authService.ts` (添加 `getLinuxDoLoginStatus()` 方法)
    -   `App.tsx` (添加 `linuxDoProfile` 状态，在初始化时检查 Linux.do 登录状态，在回调成功后保存状态)
    -   `services/linuxDoCallbackService.ts` (确保成功创建 profile 后保存到 localStorage)

---

### Fix Record - 2025-11-30 (续)

-   **Problem Description:** Linux.do OAuth token 交换时出现 CORS 错误，无法从前端直接调用 `https://connect.linux.do/oauth2/token`
-   **Root Cause:** 
    1. 浏览器 CORS 策略阻止了跨域请求
    2. Linux.do 服务器没有返回 `Access-Control-Allow-Origin` 头
    3. OAuth token 交换不应该在前端直接进行，应该通过后端代理
-   **Solution:** 
    1. **创建后端 API 代理路由**：
       - 创建 `api/linuxdo-token.ts`：代理 token 交换请求
       - 创建 `api/linuxdo-userinfo.ts`：代理用户信息获取请求
       - 两个路由都处理 CORS preflight 请求，并设置正确的 CORS 头
    2. **修改前端服务**：
       - 修改 `exchangeCodeForToken` 函数：改为调用 `/api/linuxdo-token` 而不是直接调用 Linux.do
       - 修改 `getLinuxDoUserInfo` 函数：改为调用 `/api/linuxdo-userinfo` 而不是直接调用 Linux.do
       - 请求格式从 `application/x-www-form-urlencoded` 改为 `application/json`（后端代理会转换）
-   **Impact Scope:**
    -   `api/linuxdo-token.ts` (新建文件，token 交换代理)
    -   `api/linuxdo-userinfo.ts` (新建文件，用户信息代理)
    -   `services/linuxDoAuthService.ts` (修改 token 交换和用户信息获取逻辑)

---

### Fix Record - 2025-11-30

-   **Problem Description:** Linux.do OAuth 回调返回 404 错误，无法完成登录流程
-   **Root Cause:** 
    1. 应用是 SPA（单页应用），OAuth 回调 URL `/auth/callback?code=xxx` 需要由前端 React 处理
    2. Vercel 服务器没有配置 SPA 回退路由，当访问 `/auth/callback` 时直接返回 404
    3. `vercel.json` 中只配置了 API 路由的 rewrite，缺少将其他路由指向 `index.html` 的配置
-   **Solution:** 
    1. 在 `vercel.json` 的 `rewrites` 中添加 SPA 回退路由配置
    2. 使用正则表达式 `/((?!api/).*)` 匹配所有非 API 路由
    3. 将这些路由重定向到 `/index.html`，让前端 React 应用处理路由
-   **Impact Scope:**
    -   `vercel.json` (添加 SPA 回退路由配置)
...
-   **Problem Description:** Linux.do 登录逻辑和 SQL 语句代码冗长、重复，缺乏模块化，错误处理不统一，维护困难。
-   **Root Cause:** 
    1. `linuxDoAuthService.ts` 代码冗长，包含大量重复的日志输出和错误处理
    2. SQL 函数 `create_linuxdo_profile` 缺乏参数验证和错误处理
    3. `authService.ts` 中的 `findOrCreateProfileByLinuxDoId` 函数逻辑复杂，有重复代码
    4. `App.tsx` 中的 OAuth 回调处理逻辑过长（300+ 行），难以维护
    5. 缺乏统一的错误处理和类型定义
-   **Solution:** 
    1. **重写 `linuxDoAuthService.ts`**：
       - 使用常量定义配置和存储键名，提高可维护性
       - 添加完整的 TypeScript 类型定义
       - 统一错误处理，减少冗余日志
       - 提取工具函数（`normalizeRedirectUri`、`buildRedirectUri` 等）
       - 简化函数签名，使 `redirectUri` 参数可选
       - 代码从 576 行减少到约 400 行，更清晰易读
    2. **优化 SQL 函数**（新建 `optimize_linuxdo_profile_function.sql`）：
       - 添加参数验证（检查空值）
       - 实现自动更新逻辑：如果已存在相同 `linuxdo_user_id` 的 profile，则更新而不是创建
       - 添加唯一索引防止重复
       - 改进错误处理，使用 EXCEPTION 块捕获和处理错误
       - 添加函数注释说明
    3. **优化 `authService.ts`**：
       - 简化 `findOrCreateProfileByLinuxDoId` 函数，利用优化后的 SQL 函数
       - 添加备用方法 `_fallbackFindOrCreateProfile`（当 RPC 函数不可用时）
       - 提取头像 URL 提取逻辑为独立函数
       - 改进错误处理和日志记录
    4. **创建新的回调服务**（`linuxDoCallbackService.ts`）：
       - 将 `App.tsx` 中 300+ 行的回调处理逻辑提取到独立服务
       - 统一错误处理逻辑
       - 提取辅助函数（`extractLinuxDoUserId`、`extractAvatarUrl` 等）
       - 分离 Supabase 用户和独立 Linux.do 用户的处理逻辑
       - 提供清晰的返回类型和错误信息
    5. **简化 `App.tsx`**：
       - 回调处理逻辑从 300+ 行减少到约 30 行
       - 使用新的回调服务，代码更简洁
       - 移除不再需要的导入
-   **Impact Scope:** 
    -   `services/linuxDoAuthService.ts`（完全重写，优化代码结构）
    -   `services/authService.ts`（优化 `findOrCreateProfileByLinuxDoId` 函数）
    -   `services/linuxDoCallbackService.ts`（新建文件，提取回调处理逻辑）
    -   `supabase/migrations/optimize_linuxdo_profile_function.sql`（新建文件，优化 SQL 函数）
    -   `App.tsx`（简化回调处理逻辑）

**改进效果**：
- **代码可维护性**：代码更模块化，职责分离清晰
- **代码量减少**：总体代码量减少约 200 行，但功能更完整
- **错误处理**：统一的错误处理机制，更好的错误信息
- **类型安全**：完整的 TypeScript 类型定义
- **SQL 优化**：自动处理创建和更新，防止重复数据

**代码对比**：
```typescript
// 修复前（App.tsx）：300+ 行的回调处理逻辑
// 修复后（App.tsx）：约 30 行，调用服务函数
const result = await handleLinuxDoCallback(params, currentUser);

// 修复前（SQL）：简单的插入逻辑
// 修复后（SQL）：自动处理创建和更新，添加验证
CREATE OR REPLACE FUNCTION create_linuxdo_profile(...)
  -- 自动检查是否存在，存在则更新，不存在则创建
```
...
-   **Problem Description:** Linux.do OAuth 重定向地址在前端动态构建，应该从 Supabase 数据库配置中读取，确保与 Linux.do 应用中的回调 URL 配置完全一致。
-   **Root Cause:** 
    1. `buildLinuxDoAuthUrl` 函数从前端 `window.location` 动态构建 `redirect_uri`
    2. 前端构建的 URL 可能与 Linux.do 应用中配置的回调 URL 不一致（尾部斜杠、路径等）
    3. 缺乏统一的配置管理，重定向地址应该在数据库中统一配置
-   **Solution:** 
    1. **修改配置接口**：
       - 在 `OAuthConfig` 接口中添加 `redirectUri` 字段
       - 支持从数据库读取重定向地址配置
    2. **修改配置读取逻辑**：
       - 在 `getLinuxDoConfig` 函数中添加 `redirect_uri` 的读取逻辑
       - 支持从 `oauth_config` 表的 `redirect_uri` 键读取
       - 支持从 `app_config` 表的 `linuxdo_redirect_uri` 键读取
       - 支持从环境变量 `VITE_LINUXDO_REDIRECT_URI` 读取（仅用于开发/测试）
    3. **修改 `buildLinuxDoAuthUrl` 函数**：
       - 重定向地址优先级：**数据库配置 > 参数传入 > 前端自动构建（仅作为最后备选）**
       - 优先使用数据库配置的 `redirect_uri`，确保与 Linux.do 应用配置一致
       - 如果数据库未配置，才使用参数传入或前端自动构建（并输出警告日志）
       - 添加详细的日志说明，提醒用户应该在数据库中配置
    4. **更新 SQL 迁移文件**：
       - 在 `create_oauth_config_table.sql` 中添加 `redirect_uri` 配置示例
       - 添加重要提示，说明 `redirect_uri` 必须与 Linux.do 应用中的回调 URL 完全一致
    5. **更新文档**：
       - 在 `LINUXDO_OAUTH_SETUP.md` 中添加 `redirect_uri` 配置说明
       - 强调必须在数据库中配置，避免前端动态构建导致的不一致问题
-   **Impact Scope:** 
    -   `services/linuxDoAuthService.ts`（修改配置接口和读取逻辑，优化重定向地址处理）
    -   `supabase/migrations/create_oauth_config_table.sql`（添加 redirect_uri 配置示例和说明）
    -   `@docs/LINUXDO_OAUTH_SETUP.md`（更新配置说明，强调 redirect_uri 的重要性）

**改进效果**：
- **配置统一管理**：重定向地址在数据库中统一配置，避免前端动态构建导致的不一致
- **更安全可靠**：确保重定向地址与 Linux.do 应用配置完全一致，减少 OAuth 错误
- **更好的可维护性**：所有 OAuth 配置（client_id、client_secret、redirect_uri）都在数据库中统一管理
- **向后兼容**：如果数据库未配置 redirect_uri，仍然支持前端自动构建（但会输出警告）

**配置示例**：
```sql
-- 在 Supabase 数据库中配置（推荐）
...
if (config.redirectUri) {
  finalRedirectUri = normalizeRedirectUri(config.redirectUri);  // 从数据库读取
} else if (redirectUri) {
  finalRedirectUri = normalizeRedirectUri(redirectUri);  // 参数传入（不推荐）
} else {
  finalRedirectUri = buildRedirectUri();  // 前端构建（仅作为最后备选）
}
```
...
-   **Problem Description:** 在 Supabase 数据库中已配置 Linux.do OAuth 配置，但代码仍然报错说未配置。用户无法正常使用 Linux.do 登录功能。
-   **Root Cause:** 
    1. **RLS 策略问题**：`oauth_config` 表的 RLS 策略只允许 `authenticated` 用户读取，但 Linux.do 登录流程在用户未登录时就需要读取配置
    2. **错误信息不够详细**：当配置读取失败时，错误信息没有提供足够的调试信息
    3. **缺乏详细的日志**：无法了解配置读取失败的具体原因
-   **Solution:** 
    1. **修复 RLS 策略**：
       - 修改 `create_oauth_config_table.sql`，将读取策略改为允许 `anon` 和 `authenticated` 用户读取
       - 创建新的迁移文件 `fix_oauth_config_rls_for_anonymous.sql`，用于修复现有数据库的 RLS 策略
       - OAuth 配置（client_id、redirect_uri）是公开信息，在登录流程中需要匿名用户也能读取
    2. **添加详细的日志输出**：
       - 在 `getLinuxDoConfig` 函数中添加详细的日志，包括：
         - 读取到的配置项数量和内容
         - 解析后的配置状态（是否有 client_id、client_secret、redirect_uri）
         - 错误详情（错误消息、代码、提示等）
       - 帮助用户快速定位问题
    3. **改进错误信息**：
       - 提供更详细的错误信息，包括：
       ...
       - 让用户能够快速解决问题
-   **Impact Scope:** 
    -   `services/linuxDoAuthService.ts`（添加详细日志和改进错误信息）
    -   `supabase/migrations/create_oauth_config_table.sql`（修复 RLS 策略，允许匿名用户读取）
    -   `supabase/migrations/fix_oauth_config_rls_for_anonymous.sql`（新建文件，用于修复现有数据库）

**修复步骤**：
1. **执行 RLS 策略修复**（如果数据库已创建）：
   ```sql
   ...
   CREATE POLICY "Allow anonymous users to read oauth config"
     ...
     USING (true);
   ```
   或者直接执行 `supabase/migrations/fix_oauth_config_rls_for_anonymous.sql` 文件
...
3. **检查配置数据**：
   ```sql
   -- 在 Supabase SQL Editor 中执行，检查配置是否存在
   SELECT * FROM oauth_config WHERE provider = 'linuxdo';
   ```

**常见问题排查**：
- 如果仍然报错"未配置"，请检查：
  1. 配置数据是否存在于 `oauth_config` 表中
  2. `provider` 字段是否为 `'linuxdo'`、`'slack'` 或 `'slack_oidc'`（如果 Supabase 中显示为 "Slack (OIDC)"，应使用 `'slack'` 或 `'slack_oidc'`）
  3. `key` 字段是否为 `'client_id'`、`'client_secret'`、`'redirect_uri'`（小写，下划线）
  4. RLS 策略是否已修复（允许匿名用户读取）
  5. 查看浏览器控制台的详细日志
...
-   **Problem Description:** Linux.do 在 Supabase 数据库中配置为 "Slack (OIDC)"，但代码只查找 `provider='linuxdo'` 的配置，导致无法读取配置。
-   **Root Cause:** 
    1. 代码中硬编码了 `provider='linuxdo'`，但实际数据库中可能使用 `'slack'` 或 `'slack_oidc'` 作为 provider 名称
    2. 没有支持多种 provider 名称的查找逻辑
-   **Solution:** 
    1. **修改配置读取逻辑**：
       - 支持从多个 provider 名称读取配置：`'linuxdo'`、`'slack'`、`'slack_oidc'`
       - 依次尝试这些 provider 名称，找到第一个有配置的就使用
       - 添加日志输出，显示找到的 provider 名称
    2. **更新诊断函数**：
       - `diagnoseLinuxDoConfig` 函数也支持查找多个 provider 名称
       - 在诊断结果中显示找到的 provider 名称
       - 提供更准确的建议（如果配置为 "Slack (OIDC)"，提示使用 `'slack'` 或 `'slack_oidc'`）
    3. **更新文档**：
       - 在 `LINUXDO_OAUTH_SETUP.md` 中说明支持多种 provider 名称
       - 明确说明如果 Supabase 中显示为 "Slack (OIDC)"，应使用 `'slack'` 或 `'slack_oidc'`
       - 提供配置示例
-   **Impact Scope:** 
    -   `services/linuxDoAuthService.ts`（修改配置读取逻辑，支持多种 provider 名称）
    -   `@docs/LINUXDO_OAUTH_SETUP.md`（更新配置说明，说明 provider 名称的选择）

**配置示例**：
```sql
-- 如果 Supabase 中显示为 "Slack (OIDC)"，使用以下配置：
INSERT INTO oauth_config (provider, key, value) VALUES
  ('slack', 'client_id', 'your_client_id'),
  ('slack', 'client_secret', 'your_client_secret'),
  ('slack', 'redirect_uri', 'https://xxx.supabase.co/auth/v1/callback')
ON CONFLICT (provider, key) DO UPDATE SET value = EXCLUDED.value;

-- 或者使用 'slack_oidc'：
INSERT INTO oauth_config (provider, key, value) VALUES
  ('slack_oidc', 'client_id', 'your_client_id'),
  ('slack_oidc', 'client_secret', 'your_client_secret'),
  ('slack_oidc', 'redirect_uri', 'https://xxx.supabase.co/auth/v1/callback')
ON CONFLICT (provider, key) DO UPDATE SET value = EXCLUDED.value;
...
INSERT INTO oauth_config (provider, key, value) VALUES
  ('linuxdo', 'client_id', 'your_client_id'),
  ('linuxdo', 'client_secret', 'your_client_secret'),
  ('linuxdo', 'redirect_uri', 'https://xxx.supabase.co/auth/v1/callback')
ON CONFLICT (provider, key) DO UPDATE SET value = EXCLUDED.value;
```
...
--- 
 ### Fix Record - 2026-01-04

-   **Problem Description:** Deepgram 转写结果中角色混在一起且没有标点，影响阅读体验。需要启用智能格式化和说话人检测功能，以及确保使用最新的 Nova-2 模型。
-   **Root Cause:** 
    1. 代码中已经设置了 smart_format=true 和 punctuate=true，但缺少说话人检测（diarization）功能
    2. 说话人检测可以区分不同的说话者，避免将多人对话识别成一整段文字
    3. Nova-2 模型针对多样化语境进行了优化，比旧版 base 模型表现更好
-   **Solution:** 
    1. **在 API 代理中添加 diarize 参数**：
       - 修改 pi/deepgram-proxy.ts，在查询参数中添加 diarize = 'true' 默认值
       - 将 diarize 参数添加到 URLSearchParams 中传递给 Deepgram API
    2. **在服务层的所有参数构建位置添加 diarize**：
       - 在 services/deepgramService.ts 中找到所有构建 URLSearchParams 的位置（共 5 处）
       - 在每个位置的参数对象中添加 diarize: 'true'
       - 这些位置包括：直接 API 调用、压缩音频上传、URL 模式等多种场景
    3. **确保使用 Nova-2 模型**：
       - 验证所有位置都使用 model: 'nova-2'（已经正确配置）
    4. **保持智能格式化启用**：
       - 验证 smart_format: 'true' 和 punctuate: 'true' 在所有位置都已启用（已经正确配置）
-   **Impact Scope:**
    -   pi/deepgram-proxy.ts (添加 diarize 参数支持)
    -   services/deepgramService.ts (在 5 个参数构建位置添加 diarize=true)

---

### Fix Record - 2026-01-04 (续)

-   **Problem Description:** 云端同步状态通知一直显示在屏幕右上角，无法自动关闭或手动关闭，影响用户体验。
-   **Root Cause:** 
    1. showSyncStatus 的逻辑只要有 lastSyncTime 就会一直显示通知
    2. 没有实现成功同步后的自动关闭机制
    3. 没有提供用户手动关闭通知的交互方式
-   **Solution:** 
    1. **添加通知关闭状态管理**：
       - 新增 syncNotificationDismissed 状态来跟踪用户是否已关闭通知
       - 新增 prevSyncState 状态来检测同步状态变化
       - 当同步状态变化时自动重置关闭状态，确保新的同步通知能够显示
    2. **实现 5 秒自动关闭**：
       - 使用 useEffect 监听同步完成状态（syncState === "idle" && queueLength === 0 && lastSyncTime）
       - 在成功同步后启动 5 秒倒计时，自动设置 syncNotificationDismissed 为 true
       - 如果用户已手动关闭，则不再启动自动关闭计时器
    3. **添加点击关闭功能**：
       - 为通知容器添加 onClick 事件，点击整个通知区域即可关闭
       - 添加悬停效果（hover:bg-slate-800/80）提示用户可以点击
       - 添加 	itle="点击关闭" 提示文本
    4. **添加关闭按钮**：
       - 在通知右上角添加 X 图标按钮
       - 使用 opacity-0 group-hover:opacity-100 实现鼠标悬停时才显示关闭按钮
       - 使用 e.stopPropagation() 防止按钮点击事件冒泡到容器
    5. **更新显示逻辑**：
       - 修改 showSyncStatus 条件，添加 !syncNotificationDismissed 检查
       - 确保用户关闭后通知不再显示，直到下次同步状态变化
-   **Impact Scope:**
    -   App.tsx (添加通知自动关闭和手动关闭功能)

---

### Fix Record - 2026-01-04 (续2)

-   **Problem Description:** 侧边栏一直以完全显示的状态呈现，比较明显且占用视觉空间，希望在鼠标未悬浮时自动进入隐藏态（降低透明度和饱和度），鼠标悬浮时恢复正常显示。
-   **Root Cause:** 
    1. 侧边栏组件没有实现悬浮状态检测
    2. 样式没有根据悬浮状态动态调整透明度和饱和度
    3. 缺少视觉层次感，侧边栏始终以相同的视觉权重显示
-   **Solution:** 
    1. **添加悬浮状态管理**：
       - 在 Sidebar 组件中新增 isHovered 状态来跟踪鼠标是否悬浮在侧边栏上
       - 使用 useState 初始化为 alse（默认隐藏态）
    2. **实现鼠标事件监听**：
       - 在侧边栏根容器上添加 onMouseEnter 事件，鼠标进入时设置 isHovered 为 	rue
       - 添加 onMouseLeave 事件，鼠标离开时设置 isHovered 为 alse
    3. **动态调整视觉样式**：
       - 非悬浮状态（!isHovered）：应用 opacity-30（透明度 30%）和 saturate-50（饱和度 50%）
       - 悬浮状态（isHovered）：应用 opacity-100 和 saturate-100（完全显示）
       - 使用 Tailwind 的 	ransition-all duration-300 实现平滑过渡动画
    4. **移动端适配**：
       - 添加 !isMobile 条件判断，确保移动端不应用隐藏效果
       - 移动端始终保持完全显示状态，避免交互问题
-   **Impact Scope:**
    -   components/Sidebar.tsx (添加悬浮状态管理和动态样式)
