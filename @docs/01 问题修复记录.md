...
-   **Problem Description:** 
    ...
    3. 327MB 的视频文件无法生成字幕，所有方法都失败
    
-   **Root Cause:** 
    1. Deepgram 的文件大小限制过于保守（100MB），而 Deepgram 实际上可以处理高达 2GB 的文件
    2. 当 Deepgram 因文件过大而跳过后，系统尝试使用 Gemini，但用户没有配置 Gemini API Key
    3. 智能路由的回退逻辑不够健壮，没有提供友好的错误提示告诉用户应该配置哪个 API Key
    4. FFmpeg CDN 未配置导致分块处理失败，但错误信息不够清晰
    5. 错误处理中缺少对 API Key 未配置情况的明确提示
...
-   **Solution:** 
    ...
    3. **优化最终错误提示**：
       ...
       - 如果 Deepgram 不可用，提示用户配置 Deepgram API Key（推荐）
       - 显示文件大小和可用元数据信息，帮助用户诊断问题
       - 提供具体的解决方案建议（配置 API Key、使用更小文件、启用代理等）
    
    4. **添加"无策略可用"的错误处理**：
       - 如果所有策略都无法尝试（如文件太大且没有视频元数据），提供清晰的错误信息
       - 显示文件大小、是否有视频元数据等关键信息
       - 提供针对性的解决方案建议
...
**技术细节**：
- Deepgram API 支持高达 2GB 的文件
- 将限制从 100MB 提升到 500MB，可以处理绝大多数常见视频
- 对于 327MB 的文件，现在可以直接使用 Deepgram 处理
- 错误提示分为三层：
  1. API Key 未配置错误（最常见）
  2. 所有方法都失败错误（提供诊断信息）
  3. 无策略可用错误（文件过大或缺少元数据）

**优化效果**：
- 之前：327MB 文件 → Deepgram 跳过（> 100MB）→ 分块处理失败 → Gemini 无 API Key → 失败
- 现在：327MB 文件 → Deepgram 直接处理（< 500MB）→ 成功
- 如果 Deepgram 不可用：提供清晰的错误提示，告诉用户应该配置哪个 API Key
