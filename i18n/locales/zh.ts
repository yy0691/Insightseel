...
export const zh: Translations = {
  ...
  analysisSummaryPrompt: (targetLang: string) => 
    `请分析整个视频内容（从开头到结尾），生成一份**极度简洁**的摘要（3-5句话），只提取最核心的观点和结论。\n\n要求：\n1. **覆盖整个视频**：包括开头、中间和结尾的关键内容\n2. **极度简洁**：只保留核心要点，删除所有冗余信息\n3. **突出重点**：最重要的观点、结论或行动建议\n4. **时间戳**：关键转折点使用 [HH:MM:SS] 或 [MM:SS] 格式\n5. **Markdown 格式**，语言为 ${targetLang}\n\n**禁止**：不要输出任何标题、引言、结束语或无关内容，直接输出摘要正文。`,
  
  analysisKeyInfoPrompt: (targetLang: string) => 
    `请识别视频中最重要的 5-8 个关键时刻，均匀覆盖整个视频时长（开头、中间、结尾）。\n\n格式要求：\n- 每行一个，格式：[HH:MM:SS] 简短描述（10字以内）\n- **必须覆盖整个视频**，不要只关注开头部分\n- 选择真正重要的转折点、核心观点或关键结论\n- **极度简洁**：每条描述控制在 10 个字以内\n\n示例：\n[00:15] 介绍主题\n[05:30] 核心概念讲解\n[15:45] 实际案例分析\n[22:10] 总结要点\n\n输出语言：${targetLang}。**禁止**添加任何标题、说明或额外文本。`,
  
  analysisTopicsPrompt: (targetLang: string) => 
    `请识别视频中最重要的 3-5 个核心主题，覆盖整个视频内容。\n\n要求：\n- 仅输出 Markdown 无序列表（- 主题名称）\n- **极度简洁**：每个主题 3-5 个字\n- 按重要性排序\n- **禁止**添加任何解释、说明或额外文字\n\n输出语言：${targetLang}。`,
